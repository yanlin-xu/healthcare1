<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="报告日期" prop="baogaoriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.baogaoriqi" 
                type="date"
                :readonly="ro.baogaoriqi"
                placeholder="报告日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.baogaoriqi" label="报告日期" prop="baogaoriqi">
...
export default {
  ...
  created() {
	this.ruleForm.baogaoriqi = this.getCurDate()
  ...
  methods: {
	...
	this.ruleForm.baogaoneirong = this.ruleForm.baogaoneirong.replace(reg,'../../../springbootc48o6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tijianjieguo/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tijianjieguoCrossAddOrUpdateFlag = false;
						 this.parent.search();
